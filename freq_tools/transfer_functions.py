...
class TransferFunction():
    ...
    label : str
     Optional label
    """
    def __init__(self, freqs, magnitude, phase, label='', is_in_dB=True, norm_to_dc=False,
...
class TransferFunctionModel():
    """
    Base class for transfer function models. It defines some basic behaviour. The heart of the 
    transfer function model is the `tf` method that has to be implemented by subclasses. Provides
    some basic methods like `magnitude` and `phase`.
    ...
    **kwargs :
    ...
    def plot_magnitude(self, freqs, ax=None, xscale='log', yscale='log', ylabel='magnitude'):
        """
    ...
    def plot_phase(self, freqs, ax=None, xscale='log', yscale='linear', ylabel='phase / °'):
        """
    ...
    def bode_plot(self, freqs, ax1=None, ax2=None):
        ...
        ax1, ax2 :  Axis (optional)
            If axes are provided, they will be used for the plot. if not provided, a new plot will
            be created. `ax1` is used for the magnitude, `ax2` for the phase.
...
class MachZehnderTransferFunction(TransferFunctionModel):
    ...
    def scale_noise(self, noise):
        ...
        scaled_noise = OscillatorNoise(noise.freqs, psd_phase_scaled, label=noise.label, 
            representation='psd_phase', n_sided=1)
    ...
    def plot_magnitude(self, freqs, ax=None, xscale='log', yscale='log', 
